...
calc_adx <- function(high, low, close, n = 14) {
  ...
  atr_s <- sum(tr[2:(n+1)]); apdm_s <- sum(pdm[2:(n+1)]); andm_s <- sum(ndm[2:(n+1)])
  dx_vec <- rep(NA_real_, m)
  for (i in (n+1):m) {
    if (i > n+1) {
      atr_s  <- atr_s  - atr_s/n  + tr[i]
      apdm_s <- apdm_s - apdm_s/n + pdm[i]
      andm_s <- andm_s - andm_s/n + ndm[i]
    }
    pdi <- 100 * apdm_s / atr_s; ndi <- 100 * andm_s / atr_s
    denom <- pdi + ndi
    dx_vec[i] <- if (denom == 0) 0 else 100 * abs(pdi - ndi) / denom
  }
  first <- which(!is.na(dx_vec))[1]
  if (is.na(first) || first + n > m) return(adx_vec)
  adx_s <- mean(dx_vec[first:(first+n-1)], na.rm = TRUE)
  for (i in (first+n):m) {
    adx_s <- (adx_s * (n-1) + dx_vec[i]) / n
    adx_vec[i] <- adx_s
  }
  adx_vec
}
...
for (day in as.character(trading_days)) {
  ...
  for (i in seq_len(nrow(day_data))) {
    ...
    if (row$adx >= 25) next   # solo operar en mercado ranging
