# =============================================================================
# BACKTEST EXTENDIDO: Opening Range Breakout (ORB) — SPY 5min
# Periodo: 2012-01-01 a 2026-05-02 (~14 anos)
# Descarga en chunks de ~2 anos desde Massive.com
# =============================================================================
...
# --- 1. LIBRERIAS ------------------------------------------------------------
pkgs <- c("dplyr", "lubridate", "ggplot2", "scales", "tibble", "jsonlite", "httr", "writexl")
...
# --- 3. CHUNKS DE FECHAS (~2 anos cada uno) ----------------------------------
date_chunks <- list(
  c("2012-01-01", "2013-12-31"),
  c("2014-01-01", "2015-12-31"),
  c("2016-01-01", "2017-12-31"),
  c("2018-01-01", "2019-12-31"),
  c("2020-01-01", "2021-12-31"),
  c("2022-01-01", "2023-12-31"),
  c("2024-01-01", format(Sys.Date() - 1, "%Y-%m-%d"))
)

# --- 4. FUNCION DE DESCARGA (identica al original orb_backtest_spy.R) --------
fetch_polygon_intraday <- function(symbol, from, to, api_key,
  ...
  repeat {
    ...
    n_results <- if (!is.null(data$results)) length(data$results) else 0
    cat(sprintf(" %d velas\n", n_results))
  ...
  if (length(all_results) == 0) stop("Sin datos de Massive.com para el rango solicitado.")
...
# --- 5. DESCARGAR TODOS LOS DATOS (o cargar si ya existen) -------------------
DATA_CSV  <- out("spy_5min_2012_2026.csv")
DATA_XLSX <- out("spy_5min_2012_2026.xlsx")

if (file.exists(DATA_CSV)) {
  cat("Archivo CSV de datos encontrado, cargando en lugar de descargar...\n")
  raw <- read.csv(DATA_CSV, stringsAsFactors = FALSE)
  raw$datetime <- as.POSIXct(raw$datetime, tz = "America/New_York")
  cat(sprintf("Cargadas %d velas desde CSV.\n", nrow(raw)))
} else {
  cat("\n")
  cat("=========================================================\n")
  cat("  DESCARGANDO SPY 5min DESDE Massive.com (2012-2026)\n")
  cat("  Esto tomara varios minutos por rate limits de la API\n")
  cat("=========================================================\n\n")
  ...
  for (i in seq_along(date_chunks)) {
    chunk <- date_chunks[[i]]
    cat(sprintf("--- CHUNK %d/%d: %s a %s ---\n", i, length(date_chunks), chunk[1], chunk[2]))

    tryCatch({
      chunk_data <- fetch_polygon_intraday(SYMBOL, chunk[1], chunk[2], POLYGON_KEY)
      all_data[[length(all_data) + 1]] <- chunk_data
      cat(sprintf("  OK: %d velas descargadas.\n", nrow(chunk_data)))
    }, error = function(e) {
      cat(sprintf("  ERROR: %s\n", e$message))
      cat("  Continuando con el siguiente chunk...\n")
    })

    if (i < length(date_chunks)) {
      cat("  Esperando 15s antes del siguiente chunk...\n")
      Sys.sleep(15)
    }
  }

  cat("\nCombinando y deduplicando datos...\n")
  raw <- dplyr::bind_rows(all_data)
  raw <- raw %>% dplyr::distinct(datetime, .keep_all = TRUE) %>% dplyr::arrange(datetime)

  cat(sprintf("Velas totales (combinadas): %d\n", nrow(raw)))
  ...
  # Guardar Excel
  cat("Guardando Excel...\n")
  writexl::write_xlsx(raw, DATA_XLSX)

  # Guardar CSV de respaldo
  cat("Guardando CSV de respaldo...\n")
  write.csv(raw, DATA_CSV, row.names = FALSE)

  cat("Datos guardados.\n")
}

# --- 6. FILTRAR HORARIO (09:30–15:55 ET) -------------------------------------
cat("\nFiltrando horario de trading (09:30–15:55 ET)...\n")
raw <- raw %>%
  filter(
...
cat(sprintf("Velas en horario: %d | Dias de trading: %d\n",
            nrow(raw), length(trading_days)))

if (length(trading_days) < 50) stop("Pocos dias de trading. Revisar datos.")

# --- 7. BACKTEST -------------------------------------------------------------
cat("\nEjecutando backtest ORB...\n")
trades <- list()
...
results <- dplyr::bind_rows(trades)

cat(sprintf("Total trades ejecutados: %d\n", nrow(results)))
...
# Sharpe incluye todos los dias de trading (sin trade = P&L 0)
...
results_year <- results %>%
  ...
  summarise(
    trades = n(),
    pnl_total = sum(pnl_usd),
    win_rate = mean(pnl_usd > 0) * 100,
    avg_pnl = mean(pnl_usd),
    .groups = "drop"
  )

cat("\n========================================\n")
cat("   RESULTADOS ORB BACKTEST EXTENDIDO — SPY\n")
cat("========================================\n")
cat(sprintf("Periodo:          %s a %s\n", min(results$date), max(results$date)))
cat(sprintf("Anos evaluados:   %.1f\n", length(trading_days)/252))
cat(sprintf("Dias evaluados:   %d | Con trade: %d (%.1f%%)\n",
...
cat(sprintf("P&L/ano:         $%.2f\n", total_pnl / (length(trading_days)/252)))
cat(sprintf("Max Drawdown:     $%.2f\n", max_dd))
cat(sprintf("Sharpe (anual):   %.2f\n", sharpe_annual))
cat("\nDistribucion de resultados:\n")
print(result_dist)
cat("\n--- Resultados por ano ---\n")
print(as.data.frame(results_year), row.names = FALSE)
cat("========================================\n")
...
p1 <- ggplot(results, aes(x = date, y = cum_pnl)) +
  ...
  labs(
    title    = "ORB Backtest Extendido — SPY | Curva de Equity (2012–2026)",
    subtitle = sprintf("Win Rate: %.1f%% | PF: %.2f | Sharpe: %.2f | Trades: %d | P&L: $%.0f",
                       win_rate * 100, profit_factor, sharpe_annual, n_total, total_pnl),
...
ggsave(out("orb_extended_equity_curve.png"), p1, width = 12, height = 5, dpi = 150)

# 9b. Distribucion P&L por trade
p2 <- ggplot(results, aes(x = pnl_usd, fill = pnl_usd > 0)) +
  ...
  labs(title = "Distribucion de P&L por Trade (2012–2026)", x = "P&L (USD)", y = "Frecuencia") +
  scale_x_continuous(labels = dollar_format()) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))

ggsave(out("orb_extended_pnl_dist.png"), p2, width = 8, height = 4, dpi = 150)

# 9c. P&L Long vs Short
p3 <- results %>%
  group_by(direction) %>%
  ...
  scale_fill_manual(values = c("TRUE" = "#4CAF50", "FALSE" = "#F44336")) +
  scale_y_continuous(labels = dollar_format()) +
  labs(title = "P&L Total por Direccion (2012–2026)", x = "", y = "P&L (USD)") +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))

ggsave(out("orb_extended_by_direction.png"), p3, width = 9, height = 6, dpi = 150)

# 9d. P&L por ano
p4 <- ggplot(results_year, aes(x = factor(year), y = pnl_total, fill = pnl_total > 0)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  geom_text(aes(label = sprintf("$%.0f\n(%d trades, WR %.0f%%)", pnl_total, trades, win_rate)),
            vjust = ifelse(results_year$pnl_total >= 0, -0.3, 1.3), size = 3) +
  scale_fill_manual(values = c("TRUE" = "#4CAF50", "FALSE" = "#F44336")) +
  scale_y_continuous(labels = dollar_format()) +
  labs(title = "P&L por Ano — ORB SPY", x = "Año", y = "P&L (USD)") +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))

ggsave(out("orb_extended_by_year.png"), p4, width = 10, height = 5, dpi = 150)

# --- 10. EXPORTAR TRADES -----------------------------------------------------
write.csv(results, out("orb_extended_trades.csv"), row.names = FALSE)

cat("\nArchivos generados en:", OUTPUT_DIR, "\n")
cat("  spy_5min_2012_2026.xlsx\n")
cat("  spy_5min_2012_2026.csv\n")
cat("  orb_extended_equity_curve.png\n")
cat("  orb_extended_pnl_dist.png\n")
cat("  orb_extended_by_direction.png\n")
cat("  orb_extended_by_year.png\n")
cat("  orb_extended_trades.csv\n")
cat("Done.\n")
