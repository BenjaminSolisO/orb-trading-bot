# =============================================================================
# BACKTEST: Opening Range Breakout (ORB) — QQQ 5min
...
cat("\n========================================\n")
cat("       RESULTADOS ORB BACKTEST — QQQ\n")
cat("========================================\n")
...
bh_return <- (qqq_end - qqq_start) / qqq_start * 100
bh_pnl    <- (qqq_end - qqq_start) * SHARES

cat(sprintf("\n--- Comparacion Buy & Hold ---\n"))
cat(sprintf("QQQ inicio: $%.2f | QQQ fin: $%.2f\n", qqq_start, qqq_end))
...
p1 <- ggplot(results, aes(x = date, y = cum_pnl)) +
  geom_area(fill = "#FF9800", alpha = 0.12) +
  geom_line(color = "#FF9800", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  labs(
    title    = "ORB Backtest — QQQ | Curva de Equity",
...
ggsave(out("orb_qqq_equity_curve.png"), p1, width = 12, height = 5, dpi = 150)
...
p2 <- ggplot(results, aes(x = pnl_usd, fill = pnl_usd > 0)) +
  ...
  labs(title = "Distribucion de P&L por Trade — QQQ", x = "P&L (USD)", y = "Frecuencia") +
  scale_x_continuous(labels = dollar_format()) +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))

ggsave(out("orb_qqq_pnl_dist.png"), p2, width = 8, height = 4, dpi = 150)
...
p3 <- results %>%
  ...
  scale_fill_manual(values = c("TRUE" = "#4CAF50", "FALSE" = "#F44336")) +
  scale_y_continuous(labels = dollar_format()) +
  labs(title = "P&L Total por Direccion — QQQ", x = "", y = "P&L (USD)") +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))

ggsave(out("orb_qqq_by_direction.png"), p3, width = 9, height = 6, dpi = 150)
...
p4 <- ggplot(monthly_agg, aes(x = month, y = pnl, fill = pnl >= 0)) +
  geom_col(width = 20) +
  geom_line(aes(x = month, y = cum_pnl / 10, group = 1), color = "#E65100", linewidth = 0.8, inherit.aes = FALSE) +
  ...
  labs(title = "P&L Mensual y Equity Acumulada — QQQ", x = "") +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"),
        axis.text.x = element_text(angle = 45, hjust = 1))

ggsave(out("orb_qqq_monthly_pnl.png"), p4, width = 12, height = 5, dpi = 150)
...
yearly_plot <- yearly %>%
  ...
  scale_fill_manual(values = c("TRUE" = "#4CAF50", "FALSE" = "#F44336")) +
  scale_y_continuous(labels = dollar_format()) +
  labs(title = "P&L Anual — ORB QQQ", x = "", y = "P&L (USD)") +
  theme_minimal(base_size = 13) +
  theme(plot.title = element_text(face = "bold"))

ggsave(out("orb_qqq_yearly_pnl.png"), yearly_plot, width = 10, height = 5, dpi = 150)

# --- 8. EXPORTAR -------------------------------------------------------------
write.csv(results, out("orb_qqq_trades.csv"), row.names = FALSE)

cat("\nArchivos generados en:", OUTPUT_DIR, "\n")
cat("  orb_qqq_equity_curve.png\n  orb_qqq_pnl_dist.png\n  orb_qqq_by_direction.png\n")
cat("  orb_qqq_monthly_pnl.png\n  orb_qqq_yearly_pnl.png\n  orb_qqq_trades.csv\n")
cat("Done.\n")
