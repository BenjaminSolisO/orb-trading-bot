# =============================================================================
# BACKTEST: Opening Range Breakout (ORB) — IWM 5min
# Datos: Alpaca Markets API (IEX feed)
# =============================================================================
# Misma logica ORB aplicada a Russell 2000 (small caps)
# =============================================================================
...
fetch_alpaca_bars <- function(symbol, from, to, timeframe = "5Min") {
  ...
  for (k in seq_len(n)) {
    r <- all_bars[[k]]; t_chr[k] <- r$t; o_vec[k] <- as.numeric(r$o); h_vec[k] <- as.numeric(r$h)
    l_vec[k] <- as.numeric(r$l); c_vec[k] <- as.numeric(r$c); v_vec[k] <- as.numeric(r$v)
  }
  df <- data.frame(datetime = lubridate::ymd_hms(t_chr, tz = "UTC"), open = o_vec, high = h_vec,
                   low = l_vec, close = c_vec, volume = v_vec, stringsAsFactors = FALSE)
  ...
  for (i in seq_len(nrow(post_orb))) {
    ...
    if (candle$high > orb_high && vol_ok) {
      ...
      trades[[length(trades) + 1]] <- tibble(date = as.Date(day), direction = "LONG",
        ...
        result = result, pnl_pts = exit_price - entry, pnl_usd = (exit_price - entry) * SHARES - COMMISSION,
    ...
    if (!trade_taken && candle$low < orb_low && vol_ok) {
      ...
      trades[[length(trades) + 1]] <- tibble(date = as.Date(day), direction = "SHORT",
        entry = entry, stop = stop_p, target = target, exit = exit_price,
        result = result, pnl_pts = entry - exit_price, pnl_usd = (entry - exit_price) * SHARES - COMMISSION,
        risk_pts = risk, orb_range = orb_range)
      trade_taken <- TRUE
    }
  }
...
n_total <- nrow(results); n_wins <- nrow(wins)
...
yearly <- results %>% mutate(year = year(date)) %>% group_by(year) %>%
  summarise(trades = n(), win_rate = mean(pnl_usd > 0) * 100, pnl = sum(pnl_usd),
            sharpe = if (n() > 1) mean(pnl_usd) / sd(pnl_usd) * sqrt(252) else NA,
            max_dd = min(cumsum(pnl_usd) - cummax(cumsum(pnl_usd))), .groups = "drop")

cat("\n========================================\n")
cat("       RESULTADOS ORB BACKTEST — IWM\n")
cat("========================================\n")
...
cat(sprintf("Avg Win:         $%.2f\n", avg_win)); cat(sprintf("Avg Loss:        $%.2f\n", avg_loss))
cat(sprintf("Profit Factor:   %.2f\n", profit_factor)); cat(sprintf("Total P&L:       $%.2f\n", total_pnl))
cat(sprintf("Max Drawdown:    $%.2f\n", max_dd)); cat(sprintf("Sharpe (anual):  %.2f\n", sharpe_annual))
cat("\nDistribucion:\n"); print(result_dist)
cat("\n--- Desglose por año ---\n"); print(as.data.frame(yearly), row.names = FALSE)

iwm_start <- raw %>% filter(date == min(raw$date)) %>% slice(1) %>% pull(close)
iwm_end <- raw %>% filter(date == max(raw$date)) %>% slice_tail(n = 1) %>% pull(close)
bh_return <- (iwm_end - iwm_start) / iwm_start * 100; bh_pnl <- (iwm_end - iwm_start) * SHARES
cat(sprintf("\n--- Buy & Hold ---\nIWM: $%.2f -> $%.2f | Retorno: %.1f%% | P&L: $%.2f\nORB vs B&H: +$%.2f\n",
            iwm_start, iwm_end, bh_return, bh_pnl, total_pnl - bh_pnl))
cat("========================================\n")

out <- function(fname) file.path(OUTPUT_DIR, fname)
p1 <- ggplot(results, aes(x = date, y = cum_pnl)) + geom_area(fill = "#7C4DFF", alpha = 0.12) +
  geom_line(color = "#7C4DFF", linewidth = 1) + geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  labs(title = "ORB Backtest — IWM | Curva de Equity",
       subtitle = sprintf("Win Rate: %.1f%% | PF: %.2f | Sharpe: %.2f | Trades: %d", win_rate * 100, profit_factor, sharpe_annual, n_total),
       x = "Fecha", y = "P&L Acumulado (USD)") + scale_y_continuous(labels = dollar_format()) +
  theme_minimal(base_size = 13) + theme(plot.title = element_text(face = "bold"))
ggsave(out("orb_iwm_equity_curve.png"), p1, width = 12, height = 5, dpi = 150)
...
p3 <- results %>% group_by(direction) %>% summarise(total = sum(pnl_usd), n = n(), wr = mean(pnl_usd > 0)) %>%
  ...
  labs(title = "P&L Total por Direccion — IWM", x = "", y = "P&L (USD)") +
  theme_minimal(base_size = 13) + theme(plot.title = element_text(face = "bold"))
ggsave(out("orb_iwm_by_direction.png"), p3, width = 9, height = 6, dpi = 150)

write.csv(results, out("orb_iwm_trades.csv"), row.names = FALSE)
cat("\nArchivos generados: orb_iwm_equity_curve.png, orb_iwm_by_direction.png, orb_iwm_trades.csv\nDone.\n")
